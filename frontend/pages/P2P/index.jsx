...
export default function P2P() {
    ...
    useEffect(() => {
        ...
        if (mode == 'Seller') {
            ...
            const becomeSeller = async () => {
                try {
                    setLoading(true)
                    const url = 'http://localhost:4001/user/becomeSeller';
            ...
            async function getUserRole() {
                try {
                    const response = await fetch('http://localhost:4001/user/role');
                    if (!response.ok) {
            ...
            const fetchListingsData = async () => {
                try {
                    ...
                    setLoading(true)
                    const response = await fetch('http://localhost:4001/user/listings');
                    const data = await response.json();
    ...
    async function addListing(units, pricePerUnit) {
        const url = 'http://localhost:4001/user/addListing';
        const data = {
            units: units,
